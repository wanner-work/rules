...
<Callout type="info">
  The `/types/` folder can be nested — for example
  `import SubOptions from '../types/sub/SubOptions'` is treated as a valid
  `/types/` import. The intent is to keep option contracts in one obvious
  place, no matter how deep that folder happens to be.
</Callout>
